...
// monkeyHuntingに特化したスピンオフ
// -----概要----- //
// 矢を飛ばす
// 猿アイコン、color:dkredで（とびちるパーティクルの色）一定フレーム左に動いた後自由落下。
// 自由落下のbehaviorを矢と猿に持たせる。
// 矢と猿以外のアレ（shapeとかcolor）は要らない。スピンオフなので。
// 矢は常にaimで飛ばす。
// 矢のスピードをシフトキーで変えられるようにしたら完成。
...
const AREA_HEIGHT = 480; // あとでCanvasSizeをこれよりおおきく・・もしくは横かもだけど。んー。
...
function setup(){
  ...
  // 80だけ移動して落下する。80のところ・・左から560. 線を引いておく。
  seedSet["seed" + (seedCapacity++)] = {
    x:1.0, y:0.1, shotSpeed:1, shotDirection:180, bgColor:"plgreen",
    shotShape:"monkey", shotColor:"red",
    action:{
      main:[{hide:true}, {shotAction:["set", "freeFall"]}, {fire:""}, {wait:300}, {loop:INF, back:2}],
      freeFall:[{wait:80}, {speed:["set", 0]}, {behavior:["add", "gravity"]}]
    },
    behaviorDef:{gravity:["gravity", {gravity:GRAVITY}]}
  };
...
// ---------------------------------------------------------------------------------------- //
// registUnitShapes.
// wedge → arrow, square → monkey(monkeyは回転せず猿の正方形アイコンの平行移動,当たり判定は内接円)
...
function registUnitShapes(){
  entity.registShape("wedgeSmall", new DrawWedgeShape(6, 3))
        ...
        .registShape("monkey", new DrawMonkey());
}
...
class SelfUnit{
  ...
  prepareWeapon(){
    let weaponSeed0 = {color:"black", shape:"wedgeSmall"};
    ...
    // ショット関連
    this.shotSpeed = ARROW_SPEED; // シフトキーで変更可能にする
    this.shotDirection = -90; // これは意味がないかな・・

    // 変更点：撃ちだす矢に重力加速度を付与する
    this.shotBehavior = {gravity:gravityBehavior({gravity:GRAVITY})};
  ...
  execute(){
    ...
    if(keyIsDown(32) && this.wait === 0){
      this.aimCloseEnemy(); // ここでshotDirectionをもっとも近い敵の位置の方向に修正する
      this.fire(this);
      this.wait = 60;
    }
  }
  aimCloseEnemy(){
    let minDist = INF;
    let index = -1;
    for(let i = 0; i < entity.unitArray.length; i++){
      const u = entity.unitArray[i];
      if(u.collisionFlag !== ENEMY){ continue; }
      const d = dist(this.position.x, this.position.y, u.position.x, u.position.y);
      if(minDist > d){
        minDist = d;
        index = i;
      }
    ...
    if(index < 0){ return; }
    const closeUnit = entity.unitArray[index];
    this.setShotDirection(atan2(closeUnit.position.y - this.position.y, closeUnit.position.x - this.position.x));
  }
	inFrame(){
...
class DrawWedgeShape extends DrawShape{
  ...
  draw(unit){
    ...
    quad(x + dy, y - dx,
         x - dy, y + dx,
         x - dy - dx * 5 * this.size, y + dx - dy * 5 * this.size,
         x + dy - dx * 5 * this.size, y - dx - dy * 5 * this.size);
  }
}
...
// drawMonkey.
// 改造してmonkeyにする。まずは正方形に。colliderは円で内接円。sizeは9で固定。
class DrawMonkey extends DrawShape{
  constructor(){
    super();
    this.colliderType = "circle";
    this.size = 9;
    this.life = 1; // 基礎ライフ。1にする。一撃で死ぬ。
  }
  set(unit){
    // colliderInitialize.
    unit.collider.update(unit.position.x, unit.position.y, this.size);
  ...
  draw(unit){
    const {x, y} = unit.position;
    rect(x - this.size, y - this.size, this.size * 2, this.size * 2);
  }
}
...
// gravity.
// speedは常に0で、velocityUpdateはせず、速度成分を直接いじる。
function gravityBehavior(param){
  return (unit) => {
